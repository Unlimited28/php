...
$stmt = $db->prepare('SELECT exams.*, ranks.name AS rank_name FROM exams LEFT JOIN ranks ON exams.id = ranks.id ORDER BY created_at DESC');
$stmt->execute();
$exams = $stmt->fetchAll();
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Exam Management - Super Admin</title>
            ...
            <div class="content-wrapper">
                <div class="dash-card">
                    <div class="dash-card-header">
                        <h3>All Exams</h3>
                        <a href="create_exam.php" class="btn btn-primary"><i class="ri-add-line"></i> Create New Exam</a>
                    </div>
                        ...
                        <table class="dash-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($exam['rank_name'] ?? 'N/A'); ?></td>
                                    <td><span class="status status-<?php echo $exam['status'] === 'published' ? 'approved' : 'pending'; ?>"><?php echo ucfirst($exam['status']); ?></span></td>
                                    <td>
                                        <a href="exam-questions.php?exam_id=<?php echo $exam['id']; ?>" class="btn btn-secondary btn-sm">Questions</a>
                                        <a href="edit_exam.php?id=<?php echo $exam['id']; ?>" class="btn btn-secondary btn-sm">Edit</a>
                                        <?php if ($exam['status'] === 'draft'): ?>
                                        <a href="publish_exam.php?id=<?php echo $exam['id']; ?>" class="btn btn-primary btn-sm" style="background: var(--success);">Publish</a>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
