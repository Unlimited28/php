...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email_or_id) || empty($password)) {
        $error_message = 'Please enter both email/ID and password.';
    } else {
        $db = get_db_connection();
        $stmt = $db->prepare('SELECT * FROM users WHERE email = :email OR unique_id = :unique_id');
        $stmt->execute([':email' => $email_or_id, ':unique_id' => $email_or_id]);
        $user = $stmt->fetch();
...
<head>
    ...
    <title>Login - Royal Ambassadors Portal</title>
