...
$stmt = $db->prepare('SELECT u.*, a.name as association_name FROM users u LEFT JOIN associations a ON u.association_id = a.id WHERE u.id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update_stmt = $db->prepare('UPDATE users SET full_name = ?, phone = ? WHERE id = ?');
    $update_stmt->execute([$fullname, $phone, $user_id]);
            ...
            <header class="main-header">
                ...
                <div class="header-right">
                    <div class="user-profile">
                        ...
                        <div class="user-info">
                            <span class="user-name">Pres. <?php echo htmlspecialchars($user['full_name']); ?></span>
                            <span class="user-role"><?php echo htmlspecialchars($user['association_name']); ?></span>
                        </div>
                    </div>
                </div>
            </header>
                    ...
                    <div class="dash-card-content">
                        <form class="dash-form" method="POST" action="profile_settings.php">
                            <div style="display: flex; align-items: center; gap: 24px; margin-bottom: 24px;">
                                <img src="../assets/images/director.jpg" alt="User Avatar" style="width: 80px; height: 80px; border-radius: 50%;">
